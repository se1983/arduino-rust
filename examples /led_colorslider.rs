...
impl<TC> Slider for PwmPinOps<TC, Duty = u8> {
    fn slide(&mut self, duty: u8) {
...
impl Slider for Pin<PwmOutput<Timer0Pwm>, PD6> {
    fn slide(&mut self, duty: u8) {
        let current_duty = self.get_duty();
        if current_duty < duty {
            for d in (current_duty..duty) {
...
impl Slider for Pin<PwmOutput<Timer0Pwm>, PD5> {
    fn slide(&mut self, duty: u8) {
        let current_duty = self.get_duty();
        if current_duty < duty {
            for d in (current_duty..duty) {
...
impl Slider for Pin<PwmOutput<Timer2Pwm>, PD3> {
    fn slide(&mut self, duty: u8) {
        let current_duty = self.get_duty();
        if current_duty < duty {
            for d in (current_duty..duty) {
